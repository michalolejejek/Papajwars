...
window = pyglet.window.Window(width=1248, height=900, caption="Papaj Wars", resizable=False)
...
#zniszczeni wrogowie
text1 = pyglet.text.Label("Rozjebani:", x=1000 , y=850, batch=main_batch)
text1.italic = True
text1.bold = True
text1.font_size = 16
#punkty
text2 = pyglet.text.Label("Małe dzieci:", x=1000 , y=750, batch=main_batch)
text2.italic = True
text2.bold = True
text2.font_size = 16
#animusz
text3 = pyglet.text.Label("Animusz(hp):", x=1000 , y=650, batch=main_batch)
text3.italic = True
text3.bold = True
text3.font_size = 16

#high score
high_score_text = pyglet.text.Label("Największy Zawadiaka:", x=1000 , y=450, batch=main_batch)
...
game_over_text = pyglet.text.Label("Przejebałeś!", x=600 , y=500)
...
reload_text = pyglet.text.Label("klinij r aby zaruchać ponownie", x=600 , y=350)
...
intro_text = pyglet.text.Label("Papaj invaders by artiom (spacja)", x=600 , y=450)
...
left = False
right = False
destroyed_enemies = 0 #zerowanie statystyk
next_wave = 0
score = 0
fire = False
player_fire_rate = 0
...
#tworzenie okienek poza konsolą
...
def reload():
    ...
    #załadowaie pliku z wynikami gracz
    high_score = load_high_score("res/score.txt")
...
def enemy_spawn(dt):
    ...
    if player_is_alive:
        if ufo_head_spawner <= 0:
            enemy_list.append(pyglet.sprite.Sprite(ufo_head, x=600, y=950, batch=main_batch))
        ...
        if enemy_ship_spawner <= 0:
            enemy_list.append(pyglet.sprite.Sprite(enemy_ship, x=600, y=950, batch=main_batch))
            enemy_list[-1].speed = randint(100, 300) * choice(directions)  # last spawned entity in entity list
            enemy_list[-1].hit_count = 0
...
def game_over(): 
    playsound('Dość.mp3') #dzwiek gry przegranej
